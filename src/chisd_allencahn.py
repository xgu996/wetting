...
class ChisdAllenCahn(object):
    def __init__(self, pde, mesh=None):
        ...
        self.bcs = [bcs1, bcs2]
        self.ws = [ws1, ws2]
        self.phi = [self.space.basis(self.bcs[0]),
                    self.space.face_basis(self.bcs[1])]

        # space variable
        self.S = self.space.stiff_matrix()
        self.M = self.space.mass_matrix()
        dense_mass = self.M.toarray()
        # from scipy.sparse import diags
        # self.M = diags(self.lumped_mass, format = "csr")
        ...
        self.idx = idx[flag]  # robin edge index
        ...
        self.geo2dof = [self.space.cell_to_dof(), self.space.edge_to_dof()[
            self.idx]]
    ...
    def hess_fenergy(self, uh):
        ...
        for i in range(2):
            ...
            hessval += csr_matrix((f.flat, (I.flat, J.flat)),
                                  shape=(gdof, gdof))
        return hessval
...
if __name__ == "__main__":
    ...
    data = loadmat("./test3_102/solution_data/000_UUU.mat")
